...
- **M2：容器内验收通过**（2026-08-30）——扫描/规则解析/Bangumi 索引
  （真实 dump 实测：30,745 条目，下载+导入约 71s，索引约 50–70MB）/规则匹配
  （真实数据 e2e 分流验证）/审核队列骨架/WebUI 骨架（antd 深色，
  D-040/D-041，nix 三输出全绿）/podman 脚本均已交付；
  迁移 0003（episode_end）落地。
- **当前阶段：等待 M2 宿主机测试**——按 `docs/testing/m2.md` 执行并回填
  （依赖 `ghcr.io/hinnyuu/roxy:dev` 新镜像：需先推送 feat/m2 合并后的 main）。
- 下一步动作：宿主机测试通过 → M3（LLM 层 + 多元数据源 + 审核队列交互化）。
