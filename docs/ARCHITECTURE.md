...
> ⚠️ 本节为**草案**，M1 兼容性实测（Jellyfin 10.11.11 / Emby 4.9.5）后冻结。
> 冻结前任何变更须重列实测清单。
...
| 多版本后缀 | ` [{version}]`，仅当同集存在 ≥2 版本时全体版本加后缀 | 单版本保持整洁；预期触发服务器"多版本合并"（待 M1 实测） |
| 多集合一文件 | `S01E01E02 - …`（待 M1 实测确认形式） | — |
| 电影目录/文件 | `{标题} ({年份})` | Jellyfin 电影库惯例 |
...
- [ ] `Season 00` 与 `Specials` 目录名在两家服务器的识别差异
- [ ] 同集多文件是否合并为"多版本单条目"；字幕 sidecar 是否跟随各自版本
- [ ] `S01E01E02` 多集合一识别
- [ ] `Extras/` 目录与 `-trailer` 后缀的 extras 识别行为
- [ ] 禁用刮削器后服务器对媒体目录的写行为（应保持零写入）
