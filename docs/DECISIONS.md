...
- 状态：accepted（具体模板经 M1 实测冻结，见 D-027/D-036）
...
- 状态：accepted；**命名/链接机制已由 D-038/D-039 修订**（M1/M1.5 实测
  Jellyfin 不合并多版本 → 默认改为版本仓库）。本条"并存"与"单版本无后缀"
  两项决策仍然有效
...
- 状态：accepted（规范本体已经 M1 实测冻结，见 D-036；版本后缀形态见
  D-038/D-039）
...
## D-036 M1 实测结论：命名规范主体冻结

- 状态：accepted（2026-08-30 宿主机实测，Jellyfin 10.11.11 + Emby 4.9.5，
  结果记录 docs/testing/m1.md §6）
- 冻结结论（双服务器验证通过）：
  - 剧集链接名 `S{s:02}E{e:02} - {集标题}`；
  - `Season 00` 与 `Specials` 目录名均识别为特别篇（Season 0），沿用 `Season 00`；
  - `S01E01E02` 多集合一识别为单一条目；
  - 中文标题与 Ω/＆/Δ/「」/~ 等特殊字符正常显示；
  - 剧场版 `movies/{标题} ({年份})/` 条目；
  - tvshow/episode/movie NFO 全字段读取（标题/季集/简介/放送日期/本地图片）；
  - 只读挂载零写入。
- 例外项：Emby 根目录 `-trailer` 行为（D-037）；Jellyfin 多版本不合并（D-038）。
- uniqueid 在两家"识别"页均不可见：非阻塞（roxy 持有 NFO，展示字段全部正常），
  uniqueid 继续写入，未来若需 ID 联动功能再专项探测。
- 字幕拾取与版本配套字幕：假文件无法触发播放，延至 M4 真实媒体端到端验证。

## D-037 命名红线：番外类文件禁止放剧集根目录

- 状态：accepted（依据 M1 实测）
- 背景：Emby 4.9.5 将剧集根目录的 `-trailer` 后缀文件收入第一季当作剧集；
  Jellyfin 忽略之。Emby 将 `Extras/` 目录并入特别篇，Jellyfin 识别为花絮。
- 决策：PV/CM/预告等番外文件只有两个去处：`Extras/` 子目录
  （策略 `pv_cm: extras`）或不整理（`skip`）；**永远禁止落在剧集根目录**。
  Organizer 实现时须加硬校验。
- 影响：`Extras/` 在两家行为均可接受（花絮 / 并入特别篇，内容不丢失不混入正篇）。

## D-038 Jellyfin 剧集多版本不合并：探针与兜底策略

- 状态：accepted（M1.5 探针 2026-08-30 完成，触发下述第 3 分支定稿）
- 背景：M1 实测 Jellyfin 10.11.11 将同集 3 个 `[组名]` 后缀文件解析为
  3 个重复剧集条目（集号正确但未合并）；Emby 4.9.5 合并为多版本。
  Jellyfin 电影多版本惯例为" - "分隔后缀，剧集侧需探针确认。
- 决策：
  1. M1.5 探针轮：` - 组名` / ` - 技术标签` / `[组名]` 对照，仅测 Jellyfin；
  2. 某变体合并成功 → `naming.version_suffix` 模板改用该形态；
  3. 全部失败 → 默认策略"版本仓库"：主版本进库，其余版本软链接放入
     扫描路径之外的仓库目录（位置与机制详见 D-039），WebUI 一键提拔；
     "容忍重复条目"保留为可配置模式（重复条目各自按 basename 配字幕，
     字幕配对反而天然正确）；
  4. 字幕版本配对验证与最终策略联动，延至 M4。
- **探针结果（2026-08-30，docs/testing/m1.5.md）**：三个变体全部不合并。
  另观察到 Jellyfin 将 ` - 1080p/720p` 从条目标题中剥离（按分辨率解析）
  但仍不合并。**结论：Jellyfin 10.11.11 对剧集多版本无任何合并路径**，
  按第 3 分支启用版本仓库策略，机制细节见 D-039。

## D-039 版本仓库（vault）机制

- 状态：accepted（2026-08-30，依据 D-038 探针结论；用户逐项确认）
- 决策：
  1. **位置**：`/media/vault/`（library 的兄弟目录）。媒体库只指向
     `library/tv` 与 `library/movies`，vault 天然处于扫描路径之外，
     不依赖"服务器跳过隐藏目录"这类未验证行为。目录结构镜像 library：
     `vault/tv/{番剧} ({年份})/Season XX/…`。
  2. **主版本选择**：先到者为主；同集第二个版本到达时直接进 vault。
     按压制组优先级自动选主属 YAGNI，v1 不做。
  3. **命名**：vault 模式下主版本**无后缀**（库内单一版本，天然适用
     "单版本无后缀"规则）；vault 内版本带 ` [{version}]` 后缀。
     tolerate 模式下全体版本带后缀并在库内并存。
  4. **提拔/互换**：系列页"设为主版本" = 主版本与目标 vault 版本的
     纯链接互换（台账驱动、幂等、可回滚）。
  5. **字幕跟随**：字幕链接跟随其视频链接——主版本字幕在 library，
     vault 版本的配套字幕跟随进 vault。
  6. **双服务器统一**：Emby 因此失去其原生多版本合并（每集只见主版本），
...
- 决策：
  ...
  2. flake.nix 新增 `buildNpmPackage` 派生构建 `web/`（package-lock.json +
     ...
     并以 `tags = [ "web" ]` 构建（buildGoModule 原生参数；GOFLAGS 与其内部
     变量冲突，不可直接设置）；
...
## D-042 同名多候选置信度封顶（防重制/同名作品碰撞）

- 状态：accepted（2026-08-30，M2.5 加固，用户确认）
- 背景：M2 评分中标题比对占 0.5 权重，同名不同条目（重制版如《水果篮子》
  2001/2019、《HUNTER×HUNTER》1999/2011）在 FTS 中都精确命中，取 rank 靠前者
  可得 1.0 置信度自动放行——系列级错判（收敛点会吸收后续同名文件，D-007）。
  M2 早期的"多候选并列 0.7"粗保护在精确匹配复核修复时被移除（避免多季前缀
  误降置信），防线出现缺口。
- 决策：系列解析时统计与候选归一化标题相同（name 或 name_cn 全等）的命中数；
  ≥2 个不同条目即判定同名碰撞：最终置信度封顶为 `threshold - 0.01`，强制进入
  人工队列，原因注明"同名多候选"。消歧豁免：文件名含年份（括号内 4 位数字）
  且恰有一个同名候选的 date 年份与之相符 → 碰撞解除。
- 理由：仅凭标题全等不足以支撑 1.0；年份是文件名中天然可得的廉价消歧信号；
  宁可人工一次，不可系列级批量错判。

## D-043 系列首确认（默认开启）

- 状态：accepted（2026-08-30，M2.5 加固，用户确认默认开启）
- 决策：`policy.series_first_confirm`（默认 `true`）：若某系列下尚无任何确认态
  （approved / auto_approved）placement，则当前本可自动放行的 placement 强制
  转 pending_review，原因注明"系列首次确认"；已有确认态 placement 的系列豁免。
  判定为纯状态推导（查询 placements），无 schema 变更、存量数据零迁移。
- 理由：系列级错判是批量错判（D-007 收敛点的双刃剑）；每系列一次人工确认是
  性价比最高的拦截点，与半自动定位（D-014）一致。M2 队列只读但 approve API
  可用，M3 起 UI 交互化。
- 代价：每新系列一次审核点击；自动放行率略降，换取系列级错误的早期拦截。
