...
**状态：[待验证] —— 全部以 M1 宿主机实测为准，禁止提前写进代码断言**

| # | 验证项 | 服务器 |
|---|---|---|
| 1 | `Season 00` vs `Specials` 目录识别 | Jellyfin / Emby |
| 2 | 同集多文件是否合并为"多版本单条目"；播放器内可否切换版本 | Jellyfin / Emby |
| 3 | 多版本合并时，字幕 sidecar 是否按 basename 跟随各自版本 | Jellyfin / Emby |
| 4 | `S01E01E02` 连写多集合一识别 | Jellyfin / Emby |
| 5 | `Extras/` 目录与 `-trailer` 等后缀的 extras 识别 | Jellyfin / Emby |
| 6 | 禁用刮削器 + 关闭"保存元数据到媒体目录"后，服务器对库目录是否零写入 | Jellyfin / Emby |
| 7 | NFO `<uniqueid>`（bangumi default）是否被正确读取展示 | Jellyfin / Emby |
| 8 | 相对软链接的读取/转码/字幕加载 | Jellyfin / Emby |
| 9 | 中文目录名/文件名（含全角字符、`&`、`~`）解析 | Jellyfin / Emby |
